...
const numberOfUnfundedGames = listOfUnfundedGames.length; 



// create a string that explains the number of unfunded games using the ternary operator
let displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games.
Currently, ${numberOfUnfundedGames} ${numberOfUnfundedGames == 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these
amazing games!`
...
firstGameElement.innerHTML = `<h3>${firstGame.name}</h3><p>Pledged: $${firstGame.pledged.toLocaleString('en-US')}</p>`;
firstGameContainer.appendChild(firstGameElement);

// do the same for the runner up item
const secondGameElement = document.createElement("div");
secondGameElement.innerHTML = `<h3>${secondGame.name}</h3><p>Pledged: $${secondGame.pledged.toLocaleString('en-US')}</p>`;
